...
namespace NET.W2019.Shevchenko10.TaskSort
{
    ...
    /// </summary>
    public static class BubbleDelegate
    {
        public enum Order
        {
            /// <summary>
        ...
        {
            DelegateSort(array, comparer.Compare, order);
        }
        ...
        public static void DelegateSort(int[][] array, Func<int[], int[], int> comparer, Order order)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                {
                    if ((order == Order.Ascending && comparer(array[i], array[j]) < 0)
                        || (order == Order.Descending && comparer(array[i], array[j]) > 0))
                    {
                        temp = array[i];
                        array[i] = array[j];
                        array[j] = temp;
